...
SELECT NO_CHAMBRE, NOM_CHAMBRE, NO_ETAGE, NOM_BATIMENT, TYPE_CHAMBRE, PRIX_JOUR, DT_CREATION, 1
FROM WRK.PUBLIC.CHAMBRE;

INSERT INTO WRK.PUBLIC.O_TRET (TRET_ID, MEDC_ID, MEDC_QTY, DOSG_DSC, CONS_ID, TRET_CRTN_DTTM, EXEC_ID)
SELECT t.ID_TRAITEMENT, m.MEDC_ID, t.QTE_MEDICAMENT, t.DSC_POSOLOGIE, t.ID_CONSULT, t.TS_CREATION_TRAITEMENT, 1
...
SELECT p.PART_ID, w.NOM_PERSONNEL, w.PRENOM_PERSONNEL, w.CD_STATUT_PERSONNEL, w.TS_CREATION_PERSONNEL, w.TS_MAJ_PERSONNEL, 1
...
SELECT p.PART_ID, w.NOM_PATIENT, w.PRENOM_PATIENT, 'Actif', w.TS_CREATION_PATIENT, w.TS_MAJ_PATIENT, w.DT_NAISS, w.VILLE_NAISS, w.PAYS_NAISS, w.NUM_SECU, 1
...
SELECT p.PART_ID, w.TS_DEBUT_ACTIVITE, w.TS_FIN_ACTIVITE, w.RAISON_FIN_ACTIVITE, 1
FROM WRK.PUBLIC.PERSONNEL w
JOIN WRK.PUBLIC.R_PART p ON w.ID_PERSONNEL = p.SRC_ID AND w.FONCTION_PERSONNEL = p.SRC_TYP;

INSERT INTO WRK.PUBLIC.O_TELP (PART_ID, CNTR_IND, TELP_NUM, STRT_VALD_DTTM, END_VALD_DTTM, EXEC_ID)
SELECT p.PART_ID, w.IND_PAYS_NUM_TELP, w.NUM_TELEPHONE, w.TS_CREATION_PATIENT, w.TS_MAJ_PATIENT, 1
FROM WRK.PUBLIC.PATIENT w
JOIN WRK.PUBLIC.R_PART p ON w.ID_PATIENT = p.SRC_ID AND p.SRC_TYP = 'Patient';
...
SELECT p.PART_ID, w.NUM_VOIE, w.DSC_VOIE, w.CMPL_VOIE, w.CD_POSTAL, w.VILLE, w.PAYS, w.TS_CREATION_PATIENT, w.TS_MAJ_PATIENT, 1
FROM WRK.PUBLIC.PATIENT w
JOIN WRK.PUBLIC.R_PART p ON w.ID_PATIENT = p.SRC_ID AND p.SRC_TYP = 'Patient';
...
SELECT h.ID_HOSPI, h.ID_CONSULT, h.NO_CHAMBRE, h.TS_DEBUT_HOSPI, h.TS_FIN_HOSPI, h.COUT_HOSPI, p.PART_ID, 1
...
SELECT DISTINCT 
  ...
  CD_MEDICAMENT, NOM_MEDICAMENT, CONDIT_MEDICAMENT, CATG_MEDICAMENT, MARQUE_FABRI, 1
