...
JOIN SOC.PUBLIC.R_MEDC m
  ON t.CD_MEDICAMENT = m.MEDC_CD
 AND t.CATG_MEDICAMENT = m.MEDC_CATG
 AND t.MARQUE_FABRI = m.MANF_BRND;

-- PERSONNEL vers R_PART 
INSERT INTO WRK.PUBLIC.R_PART (PART_ID, SRC_ID, SRC_TYP)
SELECT DISTINCT 
  ROW_NUMBER() OVER (ORDER BY ID_PERSONNEL, FONCTION_PERSONNEL) + (SELECT COALESCE(MAX(PART_ID),0) FROM SOC.PUBLIC.R_PART) AS PART_ID,
...
INSERT INTO WRK.PUBLIC.R_PART (PART_ID, SRC_ID, SRC_TYP)
SELECT DISTINCT 
  ROW_NUMBER() OVER (ORDER BY ID_PATIENT) + (SELECT COALESCE(MAX(PART_ID),0) FROM SOC.PUBLIC.R_PART) AS PART_ID,
  ID_PATIENT, 'Patient'
FROM WRK.PUBLIC.PATIENT;

-- PERSONNEL vers O_INDV
...
JOIN SOC.PUBLIC.R_PART p ON w.ID_PERSONNEL = p.SRC_ID AND w.FONCTION_PERSONNEL = p.SRC_TYP;
...
JOIN SOC.PUBLIC.R_PART p ON w.ID_PATIENT = p.SRC_ID AND p.SRC_TYP = 'Patient';

-- PERSONNEL vers O_STFF 
...
JOIN SOC.PUBLIC.R_PART stff ON c.ID_PERSONNEL = stff.SRC_ID AND stff.SRC_TYP <> 'Patient'
JOIN SOC.PUBLIC.R_PART patn ON c.ID_PATIENT = patn.SRC_ID AND patn.SRC_TYP = 'Patient';

-- HOSPITALISATION vers O_HOSP 
...
JOIN SOC.PUBLIC.R_PART p ON h.ID_PERSONNEL_RESP = p.SRC_ID AND p.SRC_TYP <> 'Patient';

-- MEDICAMENT vers R_MEDC (surrogate key MEDC_ID à générer)
INSERT INTO WRK.PUBLIC.R_MEDC (MEDC_ID, MEDC_CD, MEDC_NAME, MEDC_COND, MEDC_CATG, MANF_BRND, EXEC_ID)
SELECT DISTINCT 
  ROW_NUMBER() OVER (ORDER BY CD_MEDICAMENT, CATG_MEDICAMENT, MARQUE_FABRI) + (SELECT COALESCE(MAX(MEDC_ID),0) FROM SOC.PUBLIC.R_MEDC) AS MEDC_ID,
